...
import com.darkempire78.opencalculator.stealth.StealthTriggerEngine
...
class MainActivity : AppCompatActivity() {
    ...
    // === STEALTH MESSAGING SYSTEM FIELDS ===
    private lateinit var stealthTriggerEngine: StealthTriggerEngine
    private lateinit var stealthPreferences: StealthPreferences
    // ========================================
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // === STEALTH MESSAGING SYSTEM INITIALIZATION ===
        initializeStealthSystem()
        // ================================================

    }

    // === STEALTH MESSAGING SYSTEM METHODS ===

    private fun initializeStealthSystem() {
        stealthPreferences = StealthPreferences(this)

        // Setup the trigger engine with simultaneous long-press detection
        stealthTriggerEngine = StealthTriggerEngine(
            getResultDisplayText = { binding.resultDisplay.text.toString() },
            onTrigger = { launchHiddenChat() }
        )

        stealthTriggerEngine.attachToButtons(
            squareButton = binding.squareButton,
            equalsButton = binding.equalsButton
        )

        // Check for panic camouflage expression
        handlePanicCamouflage()
    }

    private fun launchHiddenChat() {
        // 🛡️ Reset the engine states so buttons don't stay locked in "pressed" mode when you return
        stealthTriggerEngine.detach()

        // Check re-entry protocol: if active session exists, fast-track
        val roomId = stealthPreferences.getSavedRoomId()
        val scrollIndex = stealthPreferences.getSavedScrollIndex()
        val fastTrack = roomId != null

        val intent = HiddenChatActivity.createIntent(
            context = this,
            roomId = roomId,
            scrollIndex = scrollIndex,
            fastTrack = fastTrack
        )
        // Launch chat without calling finish(), preserving MainActivity completely in the background task stack
        startActivity(intent)
    }

    private fun handlePanicCamouflage() {
        // Check if we returned from a panic event
        if (intent.getBooleanExtra("stealth_panic", false)) {
            val savedExpression = stealthPreferences.getLastExpression()
            if (savedExpression != null) {
                binding.input.setText(savedExpression)
                updateResultDisplay()
            }
        }
    }

    private fun checkReEntryProtocol() {
        // Check if active stealth session exists and fast-track
        if (stealthPreferences.hasActiveSession()) {
            val roomId = stealthPreferences.getSavedRoomId() ?: return
            val scrollIndex = stealthPreferences.getSavedScrollIndex()

            // Launch hidden chat directly
            val intent = HiddenChatActivity.createIntent(
                context = this,
                roomId = roomId,
                scrollIndex = scrollIndex,
                    ...
                    if (!MyPreferences(this@MainActivity).numberIntoScientificNotation || !(calculationResult >= BigDecimal(
                        ...
                        if (resultSplited.size > 1) {
                            ...
                            formattedResult = NumberFormatter.format(
                                ...
                                groupingSeparatorSymbol, numberingSystem
                            )
                        }
                    }
    ...
    override fun onResume() {
        ...
        // === STEALTH RE-ENTRY PROTOCOL ===
        // Check if an active stealth session exists and fast-track
        if (::stealthPreferences.isInitialized && stealthPreferences.hasActiveSession()) {
            checkReEntryProtocol()
        }
        // ==================================
